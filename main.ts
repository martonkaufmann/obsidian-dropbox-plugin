...
export default class DropboxPlugin extends Plugin {
	...
	async onload() {
		...
		this.registerEvent(this.app.vault.on('create', onCreate(dropbox, statusBarItemEl)))
		this.registerEvent(this.app.vault.on('modify',onModify(dropbox, statusBarItemEl)))
		this.registerEvent(this.app.vault.on('rename', onRename(dropbox, statusBarItemEl)))
		this.registerEvent(this.app.vault.on('delete',onDelete(dropbox, statusBarItemEl)))
	}
...
class DropboxPluginSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			...
			.setDesc('Access token for Dropbox account so changes can be synced (https://www.dropbox.com/developers/apps)')
			.addText(text => text
